...
let postSymbolData = (startIndex, args, bars, cb, errorCb) => {
	...
	let batchCondition = args.map(({symbol, dateRange, lastDate, timeUnit='d', dataCategory='cs', additionDate = { type:'days', value: 30 } }) => {
		...
		let begin = moment.utc(dateRange[0]).add(-1, 'minutes').toISOString();
		let end = lastDate && moment.utc(lastDate).add(1, 'minutes').toISOString() || moment.utc(dateRange[1]).add(additionDate.value, additionDate.type).toISOString();
		return {
