...
function updater(manifest, options) {
  ...
  console.log(this.options.temporaryDirectory);
}
...
updater.prototype.download = function(cb, newManifest) {
  //download path is destinationPath
...
var pUnpack = {
  ...
  mac: function(filename, cb, manifest, temporaryDirectory) {
    ...
    if (extension === ".zip") {
      console.log('unzip', filename, 'cwd',destination);
      exec('unzip -xo "' + filename + '" >/dev/null', {
        ...
        var appPath = path.join(destination, manifest.name);

        console.log('destination', destination);
        console.log('manifest', manifest);
        console.log('appPath', appPath);
        
        var dFiles = path.join(appPath, 'dFiles');
        if (fs.existsSync(dFiles)) {
          console.log("dFiles exists!");
          //fs.unlinkSync(dFiles);
        }
        cb(null, appPath);
      })
    ...
    var destinationDirectory = getZipDestinationDirectory(filename, temporaryDirectory),
      unzip = function() {
        ...
        exec('"' + path.resolve(__dirname, 'tools/unzip.exe') + '" -u -o "' +
          filename + '" -d "' + destinationDirectory + '" > NUL',
          function(err) {
            if (err) {
              return cb(err);
            }
...
var pInstall = {
   ...
   */
  mac: function(to, cb) {
    console.log("from", this.getAppPath(), "from");
...
var pInstallKS = {
   ...
   * @private
   */
  linux32: function(from, to, cb) {
    ncp(from, to, cb);
  }
};
pInstallKS.linux64 = pInstall.linux32;


//my code
updater.prototype.installKS = function(from, to, cb) {
  pInstallKS[platform].apply(this, arguments);
};
